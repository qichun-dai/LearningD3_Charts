async function drawChart() {
    ...
    let lastScrollTop = 0;  // A variable to keep track of the last scroll position

    function onScroll() {
        console.log("Scrolled!")
    const currentScrollTop = window.scrollY;
    if (currentScrollTop > lastScrollTop) {  // Detect if the scroll is going downwards
        // Define the next date you want to zoom into
        const startDate =  new Date(2007,11,1)
        const endDate =  new Date(2009,6,31)
        zoomToPeriod(startDate, endDate)
    }
    lastScrollTop = currentScrollTop;
    }

    function zoomToPeriod(startDate, endDate) {
        const duration = 750;  // Transition duration
    
        // Set the new domain based on the provided start and end dates
        xScale.domain([startDate, endDate]);
    
        // Update the x-axis and line to reflect the zoomed in period
        focus.select(".x-axis")
        ...
        // Add a text in the middle of the period
        const timeDifference = Math.abs(endDate.getTime() - startDate.getTime());
        const differenceDays = Math.ceil(timeDifference / (1000 * 3600 * 24));
        const midDate = d3.timeDay.offset(startDate, differenceDays / 2);
        focus.append('text')
            .attr('x', xScale(midDate))
            .attr('y', 0)  // Adjust this based on where you want the text
            .text(`2008 Resession: ${d3.timeFormat("%Y-%m-%d")(startDate)} to ${d3.timeFormat("%Y-%m-%d")(endDate)}`)
            .attr('fill', 'black')
            .attr('text-anchor', 'middle');
    }
